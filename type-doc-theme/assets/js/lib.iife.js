var sllib = (function (exports) {
    ...
    */
    ...
    function reverseHex62(hex62_string) {
      ...
      while (i < len) {
        origin_number += Math.pow(radix, i++) * chars.indexOf(hex62_string.charAt(len - i));
      }
    ...
    function testVs(regexp, ua) {
      return "".concat(ua.match(regexp)).replace(/[^0-9|_.]/gi, "").replace(/_/gi, ".");
    }

    function systemAbout(ua) {
      var systemName = "unknow";
      ...
      var platformName = "unknow";

      if (systemName === "windows" || systemName === "macos" || systemName === "linux") {
        platformName = "desktop"; // 桌面端
      } else if (systemName === "android" || systemName === "ios" || testUa(/mobile/gi, ua)) {
    ...
    function engineAbout(ua) {
      // 内核
      var engineName = "unknow"; // 载体

      var supporterName = "unknow";
      ...
      if (supporterName === "chrome") {
        ...
        supporterVsValue = testVs(/opera\/[\d.]+/gi, ua) !== "" ? testVs(/opera\/[\d.]+/gi, ua) : testVs(/opr\/[\d.]+/gi, ua);
    ...
    function NavigatorUserAgent(userAgent) {
      ...
      var system = systemAbout(ua);
      var engine = engineAbout(ua);
      var obj = {
